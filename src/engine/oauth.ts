/**
 * graph-memory — OAuth authentication for LLM calls
 *
 * Ported from memory-lancedb-pro/src/llm-oauth.ts
 *
 * Supports OpenAI Codex Responses API with OAuth bearer tokens
 * obtained via PKCE flow against auth.openai.com.
 */
...
/** Config-level overrides for OAuth provider endpoints (replaces process.env). */
...
// ─── Configurable overrides (no process.env) ──────────────────

function resolveOauthClientId(overrides: OAuthOverrides | undefined, providerId?: string): string {
  return overrides?.clientId?.trim() || getOAuthProvider(providerId).clientId;
}

function resolveOauthAuthorizeUrl(overrides: OAuthOverrides | undefined, providerId?: string): string {
  return overrides?.authorizeUrl?.trim() || getOAuthProvider(providerId).authorizeUrl;
}

function resolveOauthTokenUrl(overrides: OAuthOverrides | undefined, providerId?: string): string {
  return overrides?.tokenUrl?.trim() || getOAuthProvider(providerId).tokenUrl;
}

function resolveOauthRedirectUri(overrides: OAuthOverrides | undefined, providerId?: string): string {
  return overrides?.redirectUri?.trim() || getOAuthProvider(providerId).redirectUri;
}

// ─── Authorization URL builder ────────────────────────────────

function buildAuthorizationUrl(state: string, verifier: string, providerId?: string, overrides?: OAuthOverrides): string {
  const provider = getOAuthProvider(providerId);
  const url = new URL(resolveOauthAuthorizeUrl(overrides, provider.id));
  url.searchParams.set("response_type", "code");
  url.searchParams.set("client_id", resolveOauthClientId(overrides, provider.id));
  url.searchParams.set("redirect_uri", resolveOauthRedirectUri(overrides, provider.id));
...
function buildSuccessHtml(): string {
  return [
    "<!doctype html>",
    "<html><body>",
    "<h1>graph-memory OAuth complete</h1>",
...
function buildErrorHtml(message: string): string {
  return [
    "<!doctype html>",
    "<html><body>",
    "<h1>graph-memory OAuth failed</h1>",
    `<p>${message}</p>`,
    "</body></html>",
  ].join("");
}

// ─── Session extraction from JSON ─────────────────────────────
...
async function waitForAuthorizationCode(state: string, timeoutMs: number, providerId?: string): Promise<string> {
  const redirectUri = new URL(resolveOauthRedirectUri(undefined, providerId));
