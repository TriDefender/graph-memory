export interface ExtractionDrainConfig {
  maxBatchChars?: number;
  maxBatchMessages?: number;
  existingNamesMaxEntries?: number;
  existingNamesMaxChars?: number;
...
export interface ExtractionDrainPolicy {
  maxBatchChars: number;
  maxBatchMessages: number;
  existingNamesMaxEntries: number;
  existingNamesMaxChars: number;
...
export const DEFAULT_EXTRACTION_DRAIN_POLICY: ExtractionDrainPolicy = {
  maxBatchChars: 8_000,
  maxBatchMessages: 15,
...
export function normalizeExtractionDrainPolicy(
  input: ExtractionDrainConfig | undefined,
): ExtractionDrainPolicy {
  ...
  const retryDelaysMs = retryDelays.map((delay, index) => (
    integer(delay, `retryDelaysMs[${index}]`, 0, 0, 300_000)
  ));
  return {
 ...
 */
export function splitExtractionContent(content: string, maxChars: number): string[] {
